...
// Node C  ### Engineering Design group 271 
...
long interval = 5;               // Send message every 50 milliseconds   ### can be adapted ###
long duration;
...
void loop() {
  ...
  if( time_now - time_last_message >= interval )
  {
    ...
    network.update();
    RF24NetworkHeader header(nodeReceiver, 'C');                          // (Address where the data is going, Node from which it is sent)
